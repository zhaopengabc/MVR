...
static pthread_t gs_VencPid[VENC_MAX_CHN_NUM] = {0};
static SAMPLE_VENC_GETSTREAM_PARA_S gs_stPara[VENC_MAX_CHN_NUM] = {{0}};
...
HI_S32 SAMPLE_COMM_VENC_SaveH264(FILE* fpH264File, VENC_STREAM_S* pstStream)
{
    ...
    for (i = 0; i < (HI_S32)pstStream->u32PackCount; i++)
    {
...
HI_S32 SAMPLE_COMM_VENC_SaveSnap(VENC_STREAM_BUF_INFO_S* pstStreamBufInfo,VENC_STREAM_S* pstStream)
{
    ...
    FILE* pFile;
    HI_S32 s32Ret;
    pstStreamBufInfo = pstStreamBufInfo;
    ...
    switch (enType)
    {
        case PT_H264:
        {
            ...
            else if (SAMPLE_RC_VBR == enRcMode)
            {
                ...
                stH264Vbr.u32Gop = 10;   //zxr 60 frame
...
//                stH264Vbr.u32MinIQp = 10;
                stH264Vbr.u32MinQp = 10;   //10 //动态码率中限制最高质量
                stH264Vbr.u32MinIQp = 10;    //20
                //stH264Vbr.u32MaxQp = 40;
                stH264Vbr.u32MaxQp = 25;    //25 zxr 设置最大，允许动态中最低质量
                switch (enSize)
                {
                    case PIC_QCIF:
        ...
        case PT_H265:
        {
            ...
            else if (SAMPLE_RC_VBR == enRcMode)
            {
                ...
                stH265Vbr.u32MinQp  = 0;//10
                stH265Vbr.u32MinIQp = 0;//20
                //stH265Vbr.u32MaxQp  = 40;
                stH265Vbr.u32MaxQp  = 0;//25 ZXR
                switch (enSize)
                {
                    case PIC_QCIF:
...
******************************************************************************/
static void (*processH264)(int vencChn,unsigned char* buffer,int len);
void SetH264Processor(void (*processor)(int vencChn,unsigned char* buffer,int len))
{
    processH264 =processor;
}
extern int get_framerate_from_venc(int vend_ch);
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID* p)
{
    ...
    ******************************************/
    ...
    for (i = s32ChnTotal; i < s32ChnTotal+1; i++)
    {
            ...
            for (i = s32ChnTotal; i < s32ChnTotal+1; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    if(processH264)
                    {
                        //processH264(i,stStream.pstPack[i].pu8Addr + stStream.pstPack[i].u32Offset,stStream.pstPack[i].u32Len - stStream.pstPack[i].u32Offset);
                        for(HI_U32 j =0;j < stStream.u32PackCount;j++)
                        {
//                            printf("stStream.pstPack[%d].pu8Addr=%p\n",j,stStream.pstPack[j].pu8Addr);
//                            printf("stStream.pstPack[%d].u32Offset=%d\n",j,stStream.pstPack[j].u32Offset);
//                            printf("stStream.pstPack[%d].pu8Addr+u32Offset=%p\n",j,stStream.pstPack[j].pu8Addr+stStream.pstPack[j].u32Offset);
//                            printf("stStream.pstPack[%d].u32Len=%d\n",j,stStream.pstPack[j].u32Len);
//                            printf("stStream.pstPack[%d].u32Len-u32Offset=%d\n",j,stStream.pstPack[j].u32Len-stStream.pstPack[j].u32Offset);
                            processH264(i,stStream.pstPack[j].pu8Addr + stStream.pstPack[j].u32Offset,stStream.pstPack[j].u32Len - stStream.pstPack[j].u32Offset); //zxr

                            {
                                HI_S32 ret = HI_SUCCESS;
                                VENC_FRAME_RATE_S stFrameRate;
                                ret = HI_MPI_VENC_GetFrameRate( i, &stFrameRate);
                                if (HI_SUCCESS != ret)
                                 {
                                     SAMPLE_PRT("Get Venc FrameRate failed!\n");
                                     //goto END_VENC_1080P_CLASSIC_3;
                                 }
                                int fr = get_framerate_from_venc(i);
                                if(fr != stFrameRate.s32DstFrmRate)
                                {
                                    stFrameRate.s32DstFrmRate = fr; //zxr
                                    printf("================= in frm=%d,out frm=%d\n",stFrameRate.s32SrcFrmRate,stFrameRate.s32DstFrmRate);
                                    s32Ret = HI_MPI_VENC_SetFrameRate( i, &stFrameRate);
                                }
                            }
                        }
...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc_Svc_t(void* p)
{
    ...
    char szFilePostfix[10] = {0};
    VENC_CHN_STAT_S stStat = {0};
    VENC_STREAM_S stStream = {0};
    HI_S32 s32Ret = 0;
    VENC_CHN VencChn = 0;
    PAYLOAD_TYPE_E enPayLoadType[VENC_MAX_CHN_NUM] = {0};
    VENC_STREAM_BUF_INFO_S stStreamBufInfo[VENC_MAX_CHN_NUM];

    memset(&stVencChnAttr,0,sizeof(stVencChnAttr));
    memset(&read_fds,0,sizeof(read_fds));
    memset(&aszFileName,0,sizeof(aszFileName));
    memset(&stStreamBufInfo,0,sizeof(stStreamBufInfo));
    prctl(PR_SET_NAME, "hi_Proc_Svc_t", 0, 0, 0);

    pstPara = (SAMPLE_VENC_GETSTREAM_PARA_S*)p;
    s32ChnTotal = pstPara->s32Cnt;

    //usleep(s32ChnTotal*2000000);
    /******************************************
     step 1:  check & prepare save-file & venc-fd
    ******************************************/
    if (s32ChnTotal >= VENC_MAX_CHN_NUM)
                ...
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    /*******************************************************
                     step 2.3 : malloc corresponding number of pack nodes.
                    *******************************************************/
                    VENC_PACK_S *pstPack = (VENC_PACK_S*)malloc(sizeof(VENC_PACK_S) * stStat.u32CurPacks);
                    ...
                    memset(stStream.pstPack,0,sizeof(VENC_PACK_S) * stStat.u32CurPacks);    //zxr
                    /*******************************************************
                     step 2.4 : call mpi to get one-frame stream
                    *******************************************************/
                    stStream.u32PackCount = stStat.u32CurPacks;
                    //s32Ret = HI_MPI_VENC_GetStream(i, &stStream, HI_TRUE);
                    s32Ret = HI_MPI_VENC_GetStream(i, &stStream, -1);  //zxr
                    if (HI_SUCCESS != s32Ret)
                    {
                        free(stStream.pstPack);
                        stStream.pstPack = NULL;
...
//                                s32Ret = SAMPLE_COMM_VENC_SaveStream(enPayLoadType[i], pFile[i+s32Cnt], &stStream);
                                if(processH264)
                                {
                                    ...
                                    {
                                        //printf("stStream.pstPack[%d].pu8Addr=%p\n",j,stStream.pstPack[j].pu8Addr);
                                        processH264(i,stStream.pstPack[j].pu8Addr + stStream.pstPack[j].u32Offset,stStream.pstPack[j].u32Len - stStream.pstPack[j].u32Offset);
                                        //*(HI_U32*) 0 = 0;
                                    }

                                }
                                //s32Ret = SAMPLE_COMM_VENC_SaveStream(enPayLoadType[i], pFile[i+s32Cnt], &stStream);   //zxr
...
HI_S32 SAMPLE_COMM_VENC_StartGetStream(HI_S32 s32Chn)
{
    gs_stPara[s32Chn].bThreadStart = HI_TRUE;
    gs_stPara[s32Chn].s32Cnt = s32Chn;
    return pthread_create(&gs_VencPid[s32Chn], 0, SAMPLE_COMM_VENC_GetVencStreamProc, (HI_VOID*)&gs_stPara[s32Chn]);
}
...
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_StartGetStream_Svc_t(HI_S32 s32Chn)
{
    gs_stPara[s32Chn].bThreadStart = HI_TRUE;
    gs_stPara[s32Chn].s32Cnt = s32Chn;
    return pthread_create(&gs_VencPid[s32Chn], 0, SAMPLE_COMM_VENC_GetVencStreamProc_Svc_t, (HI_VOID*)&gs_stPara[s32Chn]);
}
...
HI_S32 SAMPLE_COMM_VENC_StopGetStream()
{
    ...
    {
        if (HI_TRUE == gs_stPara[i].bThreadStart)
        {
            gs_stPara[i].bThreadStart = HI_FALSE;
            if (gs_VencPid[i])
            {
                pthread_join(gs_VencPid[i], 0);
                gs_VencPid[i] = 0;
            }
        }
    }
